...
7. Do not introduce React Query / SWR for server cache: remote catalog is scrape-on-demand via `src/api/`; durable user data is SQLite.
...
- **Remote catalog**: `src/api/client.ts`, `catalog.ts`, `parse.ts` — `fetch` + win1251 HTML from `BASE_URL` (`https://newdeaf.top`).
