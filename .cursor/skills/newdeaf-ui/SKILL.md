...
name: newdeaf-ui
description: >-
  NewDeaf UI patterns for Expo screens and components: StyleSheet + src/shared/theme
...
1. Check `src/shared/ui/` before creating a new shared component.
2. Wrap scrollable/tab content with `Screen` from `@/src/shared/ui/Screen` when the existing screens already do.
3. Style with `StyleSheet.create` and tokens from `@/src/shared/theme` (`colors`, `spacing`, `radius`, `fonts`, `typography`). Never hardcode hex or spacing literals for UI chrome.
4. User-facing strings go through `t()` from `@/src/shared/i18n`. Add keys to both `ru` and `en` locale files when introducing copy.
...
8. Tablet / wide layouts: use `useBreakpoint` from `src/shared/hooks/` instead of ad-hoc width checks.
...
11. Keep `app/` routes thin — screen implementations live under `src/screens/`.

## Key paths

- Theme: `src/shared/theme/`
- i18n: `src/shared/i18n/`
- Shared UI: `src/shared/ui/` (MovieCard, MovieGrid, sheets, ConfirmDialog, EmptyState, Screen)
- Screens: `src/screens/`
...
- Fat business logic left inside `app/**` route files
